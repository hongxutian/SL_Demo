...
u8 g_KeyTable[4*Nb*(Nr+1)];// AES子密钥表，当密钥长度为16位时，占用176字节空间
u8 g_AES_IV[AES_IV_LENGTH];//分组长度必须16直接，初始化向量也必须为16字节,可以为数字数组


const u8 SBox[256] = // 加密用的SBox
{
...
void AES_Decrypt(u8 *pCipherText,u16 nDataLen,u8 *pPlainText)
{
	...
	for (i = nDataLen/(4*Nb); i > 0 ; i--, pPlainText -= 4*Nb)
	{
		...
		if (i == 1)
		{
			XorBytes(pPlainText, pIV, 4*Nb);// 最后一块数据
		}
		else
		{
